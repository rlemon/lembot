...
function generateNumbers(big, small) {
    big = Number(big);
    if (Number.isNaN(big + small)) {
        console.error(`you entered an invalid value: ${big},${small}`)
        return false;
    }
    if (big > 4) {
        console.error(`can only have 4 big, you asked for ${big}`);
        return false;
    }
    if (big + small > 6) {
        console.error(`can only have 6 numbers, you asked for ${big + small}`);
        return false;
    }
...
export const start = {
    ...
    runMatches: (matches,message,storage) => {
        ...
        if( Number(rounds) <= 0 || Number(min) <= 0 ) {
            return; // cheeky bastards
        }
